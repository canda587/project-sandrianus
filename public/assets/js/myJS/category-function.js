$(document).ready(() => {
    ...
    function category_fail(jqXHR) {
        ...
        if (data_error.category_name) {
            ...
            $("#content-form-category #infoName").html(input_info(false, data_error.category_name));
        }
        if (data_error.category_image) {
            // $("#content-form-category #name").addClass("is-invalid");
            $("#content-form-category #infoPhoto").html(input_info(false, data_error.category_image));
        }

        if (data_error.slug) {
            $("#content-form-category #slug").addClass("is-invalid");
            $("#content-form-category #infoSlug").html(input_info(false, data_error.slug));
        }
    }
    ...
    $(".show-category-update").on("click", function () {
        var btn = '<button type="button" class="action-update-category btn main-btn" style="width: fit-content">Update</button>';
        $("#categoryModalLabel").html("Update Category Data");
        $(".form-action-category .modal-body").html("Are you sure you want to update this data ?");
        $(".form-action-category #btn-action").html(btn);
        $("#categoryModal").modal("show");

        $(".action-update-category").on("click", () => {
            const static_slug = $(".form-action-category input[name=slug]").val();
            const slug = $("#content-form-category #slug").val()
            ...
            action_rest_file(data_ajax)
                .done((response) => {
                    console.log(response);
                    localStorage.setItem("alert_success", JSON.stringify(response));
                    window.location.replace(base_url(`admin/categories/${slug}/edit`));
                })
    ...
    $(".show-category-delete").on("click", function () {
        ...
        var btn = ' <button type="button" class="action-delete-category btn fail-btn" style="width: fit-content">Delete</button>';
        $("#categoryModalLabel").html("Delete Category Data");
        $(".form-action-category .modal-body").html("Are you sure you want to delete this data ?");
        $(".form-action-category #btn-action").html(btn);
        $("#categoryModal").modal("show");
